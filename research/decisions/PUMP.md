---
ticker: PUMP
name: ProPetro Holding
sleeve: power_bottleneck
current_decision: HOLD
dip_decision: RESEARCH_FIRST
sell_status: NOT_TRIGGERED
confidence_score: 30
urgency: MEDIUM
last_updated: 2026-07-12
source_classification: sa_reported_current_13f
instrument_role: common_equity
trade_policy: long_only_after_research
thesis_damage: false
unresolved_material_event: false
dip_approved: false
valuation_improved: false
portfolio_risk_allows_add: true
buy_thesis: ProPetro requires caution because the AI power link is less direct than other sleeve names and may be oilfield-services exposure.
thesis_expressed: ProPetro requires caution because the AI power link is less direct than other sleeve names and may be oilfield-services exposure.
anti_thesis: Oilfield services cyclicality may dominate any power-equipment read-through.
evidence_quality: SA_FILING_AND_LOCAL_BASELINE
valuation_case: normalized oilfield-services earnings plus any verified power-equipment option
hedge_or_sizing: No puts or shorts; use no-action discipline until valuation and thesis evidence justify capital.
invalidation_trigger: No primary-source evidence ties the business to durable AI power bottlenecks.
broken_thesis: ""
next_trigger: Next scheduled market/filing scan, SA filing change, company filing/IR update, financing or customer-contract news, guidance change, or a detected valuation dip.
one_line_rationale: "Hold/watch only: ProPetro requires caution because the AI power link is less direct than other sleeve names and may be oilfield-services exposure."
action_tier: HOLD
bottleneck_upside_score: ""
bottleneck_upside_case: ""
promotion_trigger: ""
---
# PUMP Decision

Updated: 2026-07-12

Decision: HOLD
Dip decision: RESEARCH_FIRST
Sell status: NOT_TRIGGERED
Confidence: 30 / 100
Urgency: MEDIUM

One-line rationale:
Hold/watch only: ProPetro requires caution because the AI power link is less direct than other sleeve names and may be oilfield-services exposure.

Thesis expressed:
ProPetro requires caution because the AI power link is less direct than other sleeve names and may be oilfield-services exposure.

Anti-thesis:
Oilfield services cyclicality may dominate any power-equipment read-through.

Evidence quality:
SA_FILING_AND_LOCAL_BASELINE

Hedge or sizing:
No puts or shorts; use no-action discipline until valuation and thesis evidence justify capital.

Invalidation trigger:
No primary-source evidence ties the business to durable AI power bottlenecks.

Buy now?
NO

Buy on dip?
RESEARCH_FIRST

Sell?
NO

Next trigger:
Next scheduled market/filing scan, SA filing change, company filing/IR update, financing or customer-contract news, guidance change, or a detected valuation dip.

Human action:
Follow sizing and hedge discipline before any trade.
